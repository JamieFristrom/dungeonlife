...
//     FindFirstChild(name: string, recursive?: boolean): T | undefined {
//         return this._dummyInstance.FindFirstChild(name, recursive)
//     }
...
//     WaitForChild(childName: string): T {
//         return (this._dummyInstance.WaitForChild(childName) as T)
//     }
//     // WaitForChild(childName: string, timeOut: number): T | undefined { 
//     //     return this._basePart.WaitForChild(childName, timeOut)
//     // }
...
// test death reboots hero without crashing
{
    ...
    let humanoid = (playerCharacter.FindFirstChild("Humanoid") as Humanoid|undefined)!
...
// test death reboots monster without crashing
{
    ...
    let humanoid = (playerCharacter.FindFirstChild("Humanoid") as Humanoid|undefined)!
    testContext.getPlayer().Team = (Teams.FindFirstChild("Monsters") as Team|undefined)
...
// test that MonsterAddedWait returns before too long
{
    let testContext = new TestContext()
    testContext.getPlayer().Team = (Teams.FindFirstChild("Monsters") as Team|undefined)
    const testCharacter = (ReplicatedStorage.FindFirstChild("TestObjects")!.FindFirstChild("AnimationDummy") as Model|undefined)!.Clone()
    let playerDummy = TestUtility.createTestPlayer()
    PlayerUtility.publishClientValues(playerDummy, 0, 0, "", false)
    playerDummy.Team = (Teams.FindFirstChild("Monsters") as Team|undefined)
    //const playerDummy = new Instance("Player") as Player // (ReplicatedStorage.FindFirstChild("TestObjects")!.FindFirstChild("PlayerDummy") as Folder|undefined)!.Clone() as unknown as Player
...
// test TPK
{
    ...
    if (testCharacter) {
        (testCharacter.FindFirstChild("Humanoid") as Humanoid|undefined)!.Health = 0
