...
//		descriptionFunc: ( duration: number, damage: number) => string - part of loc system now
	...
	export let enhancementFlavorInfos: {[k: string]: EnhancementFlavorInfoI} =
	{
		fire:
		{
			...
			durationFunc: ( level )=> math.sqrt( level ) * 2, // duration  // 2, 3, 3.5, 4
			damageMulFunc: ( level )=> math.sqrt( level ) * 0.20, 
...
//          3/24, nerfing all effects in half
		...
		cold:
		{
			...
			durationFunc  : ( level )=> math.sqrt( level ),  // was 0.75 increasing duration so we can actually feel it work
			effectFunc: ( level )=> 0.85 / math.sqrt( level ),  			
			// 0.85, 0.57, ..., 0.425  
...
//          3/24 nerfing all effects, from 0.25 to 0.2
		...
		explosive: 
		{
			...
			radiusFunc      : ( levelN )=> 10*math.sqrt(levelN),  // 10, 15, 18, 20
			durationFunc    : ()=> 0,
			damageMulFunc   : ( level )=> level * 0.15, 				
