...
export namespace LootServer {
	...
	export function chestDrop(targetLevel: number, player: Player) { // opening player ! currently used 
		const lootB = false
		let odds = BalanceData.chestDropRateModifierN  // / #HeroTeam:GetPlayers() // don't need to cut the chest chance by # of players because everyone has to visit
		...
		if (dieRoll <= odds) {
			let lootB = true
			const lootEventStr = LootServer.drop(targetLevel, player, boostInPlay && (dieRoll >= odds / 2))
			if (lootEventStr !== "") {
				if (boostInPlay) {
		...
		let potionB = LootServer.checkForPotionDrop(player, BalanceData.healthPotionBaseDropChance, "Healing")
		if (!potionB) {
			potionB = LootServer.checkForPotionDrop(player, BalanceData.magicPotionBaseDropChance, "Mana")
		}
		if (potionB) {
